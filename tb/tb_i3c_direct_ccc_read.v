...
module tb_i3c_direct_ccc_read;
    ...
    always @(posedge clk) begin
        if (busy) begin
            ...
            if (dut.phase == 3'd4) saw_target_read_phase <= 1'b1;
    ...
    initial begin
        ...
        saw_target_read_phase = 1'b0;
        saw_rstart_state      = 1'b0;

        $dumpfile("tb_i3c_direct_ccc_read.vcd");
        $dumpvars(0, tb_i3c_direct_ccc_read);
        ...
        if (rsp_nack || !saw_ccc_code_phase || !saw_target_addr_phase ||
            !saw_target_read_phase || !saw_rstart_state || (rsp_rx_count != 1) || (rsp_rdata[7:0] != 8'hC3)) begin
            $display("FAIL: direct read framing mismatch nack=%0d starts=%0d ccc_phase=%0d tgt_addr=%0d tgt_rd=%0d rstart=%0d rx_count=%0d data=0x%02h",
                     rsp_nack, start_count, saw_ccc_code_phase, saw_target_addr_phase,
                     saw_target_read_phase, saw_rstart_state, rsp_rx_count, rsp_rdata[7:0]);
            $finish(1);
        end
